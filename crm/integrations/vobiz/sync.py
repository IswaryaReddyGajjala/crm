...
def sync_telephony_call_logs():
	...
	try:
		...
		mongo_logs = list(db.CallLogs.find().sort("_id", -1).limit(100))
	...
	for log in mongo_logs:
		...
		# Check if already exists in Frappe CRM
		...
		# Get/Create Lead and update call properties
		lead_doc = None
		...
		if lead_doc:
			...
			campaign_name = log.get("campaign_info", {}).get("name")
			if campaign_name:
			...
			# Map Date and Time
			start_time_raw = log.get("timestamps", {}).get("start")
			if start_time_raw:
			...
			# Map Duration
			duration_val = log.get("timestamps", {}).get("duration_sec")
		...
		# Link Call Log to Lead/Contact/Deal
