  ...
  <SettingsLayoutBase>
    <template #title>
      <div class="flex gap-1 items-center">
        ...
        />
        <Badge
          v-if="vobiz.doc?.enabled && isDirty"
          :label="__('Not Saved')"
          variant="subtle"
          theme="orange"
        />
    ...
    <template #header-actions>
      <div v-if="vobiz.doc?.enabled && !vobiz.get.loading" class="flex gap-2">
        <Button
          v-if="isDirty"
    ...
    <template #content>
      <div v-if="vobiz.doc" class="h-full">
        <div v-if="vobiz.doc.enabled" class="space-y-4">
          ...
          <div class="flex items-center justify-between">
            ...
            <div>
              <Switch v-model="vobiz.doc.record_calls" size="sm" />
            </div>
          </div>
          <div class="h-px border-t border-outline-gray-modals" />
            ...
            <div v-if="vobiz.originalDoc?.auth_id" class="space-y-3">
              <div class="flex justify-between items-center">
                ...
                <Button
                  ...
                  variant="subtle"
                  :loading="vobiz.fetchVobizApps?.loading"
                  @click="fetchApps"
                />
              </div>
              ...
              <div v-if="vobizApps.length > 0" class="flex gap-4 items-end">
                ...
                <Button
                  ...
                  variant="solid"
                  :loading="vobiz.registerWebhook?.loading"
...
import { Switch, Badge, Button, FormControl, LoadingIndicator, toast } from 'frappe-ui'
...
async function fetchWebhookUrl() {
  try {
    const url = await vobiz.getWebhookUrl.submit()
    if (url) {
...
async function fetchApps() {
  try {
    const res = await vobiz.fetchVobizApps.submit()
    let apps = []
...
async function registerWebhook() {
  ...
  try {
    const res = await vobiz.registerWebhook.submit({ app_id: vobiz.doc.app_id })
...
function enable() {
  vobiz.doc.enabled = true
  update()
}

function disable() {
  vobiz.doc.enabled = false
  update()
}
